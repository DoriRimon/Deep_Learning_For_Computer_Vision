...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        flatten_x_train_i = x_train[i].flatten()
        flatten_x_test_j = x_test[j].flatten()
        dists[i, j] = ((flatten_x_train_i - flatten_x_test_j) ** 2).sum()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      flatten_x_train_i = x_train[i].flatten()
      flatten_size = flatten_x_train_i.shape[0]
      broadcased_flatten = flatten_x_train_i.broadcast_to((num_test, flatten_size))
      dists[i] = ((x_test.view(num_test, flatten_size) - broadcased_flatten) ** 2).sum(dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # get the number of elements per data point
    flattened_size = torch.numel(x_train[0])

    # flatten train and test to (#point, flattened_size)
    view_train = x_train.view(num_train, flattened_size)
    view_test = x_test.view(num_test, flattened_size)

    # we only need the diagonals of this matrices
    view_train_squared = view_train.mm(view_train.t())  # num_train x num_train
    view_test_squared = view_test.mm(view_test.t())  # num_test x num_test

    train_diag = view_train_squared[[i for i in range(num_train)], [i for i in range(num_train)]].view(num_train, 1)  # diagonal
    test_diag = view_test_squared[[i for i in range(num_test)], [i for i in range(num_test)]].view(1, num_test)  # diagonal

    train_broadcast = train_diag.broadcast_to(num_train, num_test)
    test_broadcast = test_diag.broadcast_to(num_train, num_test)

    dists = train_broadcast + test_broadcast
    dists -= 2 * torch.mm(view_train, view_test.t())
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      top_k_values, top_k_indices = torch.topk(dists[:, i], k, largest=False)
      top_k_labels = y_train[top_k_indices]
      sorted_labels, _ = top_k_labels.sort(descending=False)
      prev_value = sorted_labels[0]
      pred_i = prev_value
      current_count = 1
      max_count = 1
      for j in range(1, sorted_labels.shape[0]):
        new_value = sorted_labels[j]
...
def knn_cross_validate(
    ...
    for k in k_choices:
      for i in range(num_folds):
        cur_x_test, cur_y_test = x_train_folds[i], y_train_folds[i]
        
        cur_x_train = torch.cat([x_train_folds[j] for j in range(num_folds) if j != i], dim=0)
        cur_y_train = torch.cat([y_train_folds[j] for j in range(num_folds) if j != i], dim=0)

        cur_knn_classifier = KnnClassifier(cur_x_train, cur_y_train)
        cur_acc = cur_knn_classifier.check_accuracy(cur_x_test, cur_y_test, k, quiet=True)

        if k not in k_to_accuracies:
          k_to_accuracies[k] = dict()
        k_to_accuracies[k][i] = cur_acc
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accs in sorted(k_to_accuracies.items()):
      mean_acc = float(torch.mean(torch.Tensor(list(k_to_accuracies[k].values()))))
