...
def create_sample_tensor() -> Tensor:
    ...
    x = torch.tensor([[0 for _ in range(2)] for _ in range(3)])
    x[0, 1] = 10
    x[1, 0] = 100
    ###########################################################################
    #                            END OF YOUR CODE                             #
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    start_offset = start % 10
    start_10 = start + (10 - start_offset) if start_offset != 0 else start

    stop_offset = stop % 10
    stop_10 = stop - stop_offset if stop_offset != 0 else stop

    print(start_10, stop_10)
    if stop_10 - start_10 < 10:
      x = torch.tensor([], dtype=torch.float64)
    else:
      x = torch.arange(start_10, stop_10 + 1, 10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = x[list(torch.arange(x.shape[0]))[::-1],:]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    row_mins_vals, row_min_idxs = x.min(dim=1)
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mean_per_col = x.sum(dim=0) / x.shape[0]

    std_per_col = x - mean_per_col.view(1, x.shape[1])
    std_per_col **= 2 
    std_per_col = std_per_col.sum(dim=0) / (x.shape[0] - 1)
    std_per_col = std_per_col.sqrt()

    y = x - mean_per_col.view(1, x.shape[1])
    y /= std_per_col.view(1, x.shape[1])
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
